...
use crate::db::{AuthDB, CaptchaDB, CookieDB, MoneyDB, DB};
...
impl Display for InvalidUserReason {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		match self {
			UsernameLenIncorrect => write!(
				f,
				"The username given is too short! (Must be between {} and {} long",
				MIN_USER_LEN, MAX_USER_LEN
			),
			NotAlphanumeric => write!(f, "The username given is not alphanumeric"),
			ReservedUsername => write!(
				f,
				"The username given is one of the following reserved usernames: {:?}",
				RESERVED_USERNAMES
			),
		}
	}
...
pub async fn register(
	auth_req: AuthRequest, auth_db: AuthDB, money_db: MoneyDB, auth_cookie_db: CookieDB,
	socket_addr: Option<SocketAddr>, logger: Sender<Log>, captcha_db: CaptchaDB,
	...
	let get_res = async move {
		match auth_db.contains_key(&username).unwrap() {
			true => {
			...
			false => {
				let captcha_hash =
					match hex_simd::decode_to_boxed_bytes(captcha_guess.hash.as_bytes()) {
				...
				if let Some(reason) = invalid_user_reason(&username) {
					let login_req_denial =
						RequestDenial::new(DenialFault::User, reason.to_string(), String::new());
				...
				let correct_captcha = match captcha_db
					...
					.unwrap()
				{
					Some(captcha) => {
						if captcha_guess.answer == captcha.answer {
							captcha_db
								.remove(captcha_hash.as_ref().try_into().unwrap())
								.unwrap()
								.unwrap();
							true
						} else {
				...
				money_db
					.insert(&user.username, &FinancialInfo::new(addr_index, addr))
					.unwrap();

				let cookie = generate_auth_cookie(&user.username, &auth_db, auth_cookie_db.clone());

				Ok(auth_cookie_result_to_response(
					user.username,
					cookie,
					&logger,
					socket_addr,
	...
	let (res, _) = tokio::join!(get_res, tokio::time::sleep(Duration::from_millis(100)));

	res
}

pub(crate) async fn login(
	auth_req: AuthRequest, auth_db: AuthDB, auth_cookie_db: CookieDB,
	socket_addr: Option<SocketAddr>, logger: Sender<Log>, captcha_db: CaptchaDB,
) -> Result<impl Reply, Rejection> {
	let username = auth_req.username.to_lowercase();
	let password = auth_req.password;
	...
	let give_response = async move {
		match auth_db.get(&username).unwrap() {
			Some(user_info) => {
				let captcha_hash =
					hex_simd::decode_to_boxed_bytes(captcha_guess.hash.as_bytes()).unwrap();
				let correct_captcha = match captcha_db
					.get(captcha_hash.as_ref().try_into().unwrap())
					.unwrap()
				{
				...
				.unwrap()
				{
					...
					true => {
						let auth_cookie =
							generate_auth_cookie(&username, &auth_db, auth_cookie_db.clone());
...
fn generate_auth_cookie(
	username: &String, auth_db: &AuthDB, auth_cookie_db: CookieDB,
) -> Result<AuthCookie, GenAuthCookieErr> {
...
async fn auth_cookie_result_to_response(
	username: String, auth_cookie: Result<AuthCookie, GenAuthCookieErr>, logger: &Sender<Log>,
	socket_addr: Option<SocketAddr>, login: bool,
) -> Response<String> {
...
pub fn view_captcha(hash: String, captcha_db: CaptchaDB) -> Response<Vec<u8>> {
	...
	let captcha = captcha_db
		.get(hash_bytes.as_ref().try_into().unwrap())
		.unwrap();
...
pub(crate) fn verify_auth_cookie(
	username: &String, cookie: &str, auth_cookie_db: &CookieDB,
) -> bool {
	match auth_cookie_db.get(username).unwrap() {
		Some(auth_cookie) => auth_cookie.cookie.as_str() == cookie,
		None => false,
	}
}
